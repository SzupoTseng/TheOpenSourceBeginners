...
BOOK_TITLE = "オープンソースの目利き"
BOOK_SUB   = "初心者からアーキテクトへ、オープンソース選定の聖典 — The Open Source Beginners"
...
def title_for(base, text):
    ...
    m = re.search(r'^#\s+(.*)$', text, re.M)   # 各ファイルの H1 = 第N部／導読／付録
    return m.group(1).strip() if m else base

def part_for(text):
    return None   # 各ファイル自体が一「部」。ナビはフラットに列挙
...
def build_merged():
    ...
    parts.append("> 合本版。本書は教育・研究を目的とする——2026 年の最も話題／最多利用／急上昇の"
                 "オープンソース 196 プロジェクトの徹底解剖と選定の指針である。\n")
    parts.append("\n---\n\n## 目次\n")
    for base in FILES:
