...
# -*- coding: utf-8 -*-
"""组装《开源器识》成品:
  1) 合并单一 Markdown 档  开源器识_全书.md
  2) 图文 HTML(侧边子目录)  开源器识.html
版面沿用《代码壁垒》/《代理觉醒》_build.py(同一套 CSS / 解析器),
内容为 196 个开源项目的深度解剖与选型圣经。
...
BOOK_TITLE = "开源器识"
BOOK_SUB   = "一本从新手到架构师的开源选型圣经 · The Open Source Beginners"
COVER_CANDIDATES = ["TheOpenSourceBeginners.png", "开源器识.png", "cover.png"]
...
def title_for(base, text):
    ...
    m = re.search(r'^#\s+(.*)$', text, re.M)   # 本书每档 H1＝第N篇／导读／附录，直接取首个 H1
    return m.group(1).strip() if m else base

def part_for(text):
    return None   # 本书每档即一「篇」，nav 逐档列出，不另插分组分隔
...
def build_merged():
    ...
    parts.append("> 全书合并版。本书为教育与研究用途，是 196 个最红／最多人用／最新热度开源项目的深度解剖与选型圣经。\n")
    parts.append("\n---\n\n## 目录\n")
    for base in FILES:
